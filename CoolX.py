...
with tabs[0]:
    ...
    with col_map_render:
        ...
        # NOTE: st.plotly_chart silently keeps a map's *previous* viewport across reruns
        # unless the component is force-remounted with a fresh `key` (a known
        # Streamlit/Plotly interaction quirk) - this is why the map was staying in
        # place even though center/zoom were being recalculated correctly above.
        # Tying the key to the current coordinates + layer guarantees a clean remount
        # (and a real jump to the new location) every time either one changes.
        map_widget_key = f"hotspot_map_{round(st.session_state.map_center_lat, 4)}_{round(st.session_state.map_center_lon, 4)}_{map_layer_view}"
        st.plotly_chart(fig_hotspot, use_container_width=True, key=map_widget_key)
