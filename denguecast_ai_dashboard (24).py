...
if search_keyword:
    try:
        headers = {"User-Agent": "GA1A-Climate-Infectious-Disease-Tracker-ACTION2026 (contact: ga1a-support@example.com)"}
        ...
        if r.status_code == 200:
            ...
            if not suggestions:
                geocode_error = f"No geocoding matches found for '{search_keyword}'."
        else:
            geocode_error = f"Geocoding service returned HTTP {r.status_code}."
    except Exception as e:
        geocode_error = f"Geocoding request failed ({type(e).__name__}) - check network/firewall access to nominatim.openstreetmap.org."

if not suggestions:
    suggestions = [{"display": "Kuala Terengganu, Terengganu, Malaysia (Default)", "lat": 5.33, "lon": 103.11}]

# Surface the fallback visibly: this is the #1 reason the map/hotspots can look "stuck" on one
# place - if geocoding silently fails, current_lat/current_lon never change no matter what is
# searched, so nothing downstream (map center, shaded hotspots) has any reason to move either.
if geocode_error:
    st.sidebar.warning(f"⚠️ {geocode_error} Falling back to the default location below.")
...
# Manual "force redraw" escape hatch: bumps a nonce that feeds into the map widget's key
# (see TAB 1 map render) so the chart is guaranteed to fully remount even on the rare chance
# the automatic lat/lon-based key didn't change enough to trigger Streamlit to redraw it.
if "map_force_redraw_nonce" not in st.session_state:
    st.session_state.map_force_redraw_nonce = 0
if st.sidebar.button(lang['realign_btn'], use_container_width=True, help="Click if the map/hotspots ever look stuck after changing location."):
    st.session_state.map_force_redraw_nonce += 1
...
st.sidebar.caption(f"📍 Resolved coordinates: {current_lat:.4f}, {current_lon:.4f}")
...
# CALIBRATION: the raw product above frequently exceeds 1.0 even under "normal" tropical
# defaults (humidity_factor and rain_factor both regularly run >1, hazard runs up to 0.84),
# which was pinning final_risk_score at the 100% cap for almost every location/disease
# combination -> the LED panel always rendered CRITICAL regardless of inputs.
# Dividing by this calibration constant rescales the product back into a range where
# STABLE / ESCALATING / CRITICAL are all genuinely reachable from realistic inputs.
RISK_CALIBRATION_FACTOR = 1.6
final_risk_score = min(100, max(5, int((computed_risk * 100) / RISK_CALIBRATION_FACTOR)))
...
with tabs[0]:
    ...
    with col_map_render:
        ...
        # Build one dynamic key/revision string from everything that should make the map
        # "jump": the resolved coordinates, the active layer filter, and the manual
        # force-redraw nonce from the sidebar button. Setting BOTH `key` (forces Streamlit
        # to fully remount the component) AND `uirevision` (tells Plotly.js itself to drop
        # any cached viewport state) is belt-and-suspenders against the map/hotspot shading
        # silently keeping a stale view across reruns.
        map_revision = f"{round(st.session_state.map_center_lat, 4)}_{round(st.session_state.map_center_lon, 4)}_{map_layer_view}_{st.session_state.map_force_redraw_nonce}"
        ...
        fig_hotspot.update_layout(
            mapbox=dict(
                ...
                center={"lat": st.session_state.map_center_lat, "lon": st.session_state.map_center_lon},
                zoom=st.session_state.map_zoom,
                layers=mapbox_layers  # Injects the geometrically scaling circle layouts
            ),
            uirevision=map_revision,
        ...
        # NOTE: st.plotly_chart silently keeps a map's *previous* viewport (and previous
        # custom mapbox_layers/shaded hotspot circles) across reruns unless the component
        # is force-remounted with a fresh `key` (a known Streamlit/Plotly interaction
        # quirk) - this is why the map and the shaded hotspot circles were staying in
        # place even though center/zoom/layers were being recalculated correctly above.
        st.plotly_chart(fig_hotspot, use_container_width=True, key=f"hotspot_map_{map_revision}")
    ...
    with col_dashboard_lights:
        ...
        # LED order: red=critical, yellow=escalating, green=stable.
        # IMPORTANT: every line below is flush against column 0 on purpose.
        # Markdown treats any line indented 4+ spaces as a literal code block,
        # so matching this string's indentation to the surrounding Python code
        # (as it was before) made Streamlit render the raw HTML as text instead
        # of parsing it - that's why the panel was showing as a code block.
        led_panel_html = (
            '<div style="background: rgba(15, 23, 42, 0.55); border: 1px solid rgba(255, 255, 255, 0.08); '
            'border-radius: 16px; padding: 24px; text-align: center; min-height: 400px; display: flex; '
            'flex-direction: column; justify-content: space-around;">'
            '<div>'
            '<p style="font-size: 11px; font-family: \'Space Mono\', monospace; color: #94a3b8; '
            'letter-spacing: 0.1em; margin: 0 0 15px 0;">BIO-SECURITY STATUS LIGHTS</p>'
            f'<div style="margin: 15px auto; width: 45px; height: 45px; border-radius: 50%; transition: all 0.3s; {red_led_style}"></div>'
            f'<div style="margin: 15px auto; width: 45px; height: 45px; border-radius: 50%; transition: all 0.3s; {yellow_led_style}"></div>'
            f'<div style="margin: 15px auto; width: 45px; height: 45px; border-radius: 50%; transition: all 0.3s; {green_led_style}"></div>'
            '</div>'
            ...
            f'display: block; letter-spacing: 0.15em;">ACTIVE THREAT INDEX</span>'
            f'<span style="font-size: 26px; font-weight: 800; color: {risk_color}; letter-spacing: -0.03em;">{risk_level}</span>'
            '</div>'
            '</div>'
        )
        st.markdown(led_panel_html, unsafe_allow_html=True)
