...
# Center core location matching active coordinate geocoding
...
# Continuous Color Scale representing the physical Risk Heat Index globally
fig_hotspot = px.scatter_mapbox(
    ...
    color="Risk Score (%)",
    size="Risk Score (%)",
    color_continuous_scale=["#00d2ff", "#ffd000", "#ff2e93"],
    size_max=45,
...
# --- 7. TELEMETRY INSPECTOR & FORECAST GRAPH (UX UPGRADE) ---
st.markdown("---")
col_inspect, col_graph = st.columns([1.5, 1.5])
...
with col_inspect:
    ...
    inspected_place = st.selectbox(
        "Select active hotspot node on the map to query parameters & history:",
    ...
    st.markdown(f"""
    <div style='background: rgba(15, 23, 42, 0.45); border: 1px solid rgba(255, 255, 255, 0.08); padding: 20px; border-radius: 16px; margin-top: 10px; box-shadow: 0 8px 32px rgba(0,0,0,0.3);'>
        ...
        <div style='display: grid; grid-template-columns: repeat(4, 1fr); gap: 12px;'>
            <div style='background: rgba(10,15,30,0.6); padding: 10px; border-radius: 10px; text-align:center; border: 1px solid rgba(255,255,255,0.04);'>
                <span style='font-size: 9px; color:#94a3b8; text-transform:uppercase;'>TEMP</span>
                <p style='font-size:14px; font-weight:800; color:#00d2ff; margin:3px 0 0 0;'>{node_temp}°C</p>
            </div>
            <div style='background: rgba(10,15,30,0.6); padding: 10px; border-radius: 10px; text-align:center; border: 1px solid rgba(255,255,255,0.04);'>
                <span style='font-size: 9px; color:#94a3b8; text-transform:uppercase;'>HUMIDITY</span>
                <p style='font-size:14px; font-weight:800; color:#00d2ff; margin:3px 0 0 0;'>{node_humidity}%</p>
            </div>
            <div style='background: rgba(10,15,30,0.6); padding: 10px; border-radius: 10px; text-align:center; border: 1px solid rgba(255,255,255,0.04);'>
                <span style='font-size: 9px; color:#94a3b8; text-transform:uppercase;'>PRECIP</span>
                <p style='font-size:14px; font-weight:800; color:#00d2ff; margin:3px 0 0 0;'>{node_precip:+} %</p>
            </div>
            <div style='background: rgba(10,15,30,0.6); padding: 10px; border-radius: 10px; text-align:center; border: 1px solid rgba(255,255,255,0.04);'>
                <span style='font-size: 9px; color:#94a3b8; text-transform:uppercase;'>NODE RISK</span>
                <p style='font-size:14px; font-weight:800; color:#ff2e93; margin:3px 0 0 0;'>{node_risk}%</p>
            </div>
        </div>
    </div>
    """, unsafe_allow_html=True)
...
with col_graph:
    ...
    # Calculate a non-linear 4-week risk trajectory mapping the disease propagation curve
    ...
    wk4_risk = min(100, max(5, int(wk3_risk * 0.85))) # Natural biological decline of vector population
...
# --- 7.5 GROUND-TRUTH VISUAL INSPECTION LENS (KILLER B2G SATELLITE FEATURE) ---
st.markdown("---")
st.markdown("<h3 style='font-size:22px; font-weight:800; letter-spacing:-0.02em;'>🛰️ GA1A Visual Ground-Truth Lens</h3>", unsafe_allow_html=True)

col_lens_map, col_lens_actions = st.columns([1.8, 1.2])

with col_lens_map:
    # We dynamically embed an OpenStreetMap satellite/street visual frame exactly centered on selected node
    osm_embed_url = f"https://www.openstreetmap.org/export/embed.html?bbox={row_details['lon']-0.005}%2C{row_details['lat']-0.003}%2C{row_details['lon']+0.005}%2C{row_details['lat']+0.003}&layer=mapnik&marker={row_details['lat']}%2C{row_details['lon']}"
    st.components.v1.iframe(osm_embed_url, height=350, scrolling=False)

with col_lens_actions:
    st.markdown(f"""
    <div style='background: rgba(15, 23, 42, 0.45); border: 1px solid rgba(255, 255, 255, 0.08); padding: 24px; border-radius: 16px; min-height: 350px;'>
        <h4 style='color:#00d2ff; font-weight:700; margin-top:0;'>🛠️ Ground-Truth Tactical Command</h4>
        <p style='font-size:13px; color:#94a3b8; line-height:1.6;'>
            Connect directly to Google Maps engines to trigger Street View spatial visual sweeps or dispatch sanitisation squads to coordinate:
            <br><code style='color:#ff2e93;'>{row_details['lat']:.5f}, {row_details['lon']:.5f}</code>
        </p>
    """)
    
    # Construction of precision Google Maps launch URLs
    ...
    st.link_button("🌐 Open Google Street View Ground Lens", gmaps_streetview_url, use_container_width=True)
    st.link_button("🛰️ Open Google Maps Satellite Overlay", gmaps_sat_url, use_container_width=True)
    
    st.markdown("---")
    if st.button("📋 Dispatch Field Vector Control Squad", use_container_width=True):
        st.success(f"Command Issued: Sanitization & biology squads dispatched to inspect {inspected_place} corridor.")
    st.markdown("</div>", unsafe_allow_html=True)


# --- 7.8 PATHOLOGY RELATIONSHIP CASCADES (SYSTEMS THINKING) ---
st.markdown("---")
...
# Based on selected humidity and precipitation parameters, show linked pathogen risks
with col_cas_1:
    ...
    <div style='background: rgba(15, 23, 42, 0.35); border: 1px solid rgba(255, 255, 255, 0.05); padding: 16px; border-radius: 12px;'>
        ...
        <p style='font-size: 20px; font-weight: 800; color: #ff2e93; margin: 4px 0;'>{linked_dengue_risk}% Secondary Risk</p>
...
with col_cas_2:
    ...
    <div style='background: rgba(15, 23, 42, 0.35); border: 1px solid rgba(255, 255, 255, 0.05); padding: 16px; border-radius: 12px;'>
        ...
        <p style='font-size: 20px; font-weight: 800; color: #ffd000; margin: 4px 0;'>{linked_water_risk}% Secondary Risk</p>
        <div style='background: rgba(255,255,255,0.05); border-radius: 4px; height: 6px; overflow: hidden;'>
...
with col_cas_3:
    ...
    st.markdown(f"""
    <div style='background: rgba(15, 23, 42, 0.35); border: 1px solid rgba(255, 255, 255, 0.05); padding: 16px; border-radius: 12px;'>
        <p style='font-size: 11px; font-weight:bold; color:#94a3b8; margin:0;'>🐀 FLOODING VECTOR PATHWAYS (LEPTOSPIROSIS)</p>
        <p style='font-size: 20px; font-weight: 800; color: #00f5a0; margin: 4px 0;'>{linked_leptospirosis}% Secondary Risk</p>
        <div style='background: rgba(255,255,255,0.05); border-radius: 4px; height: 6px; overflow: hidden;'>
...
# --- 8.5 EXPLAINABLE AI EXECUTIVE SUMMARY BRIEF (SaaS MUST-HAVE) ---
st.markdown("---")
...
# Constructing a dynamic intelligence summary based on parameter anomalies
